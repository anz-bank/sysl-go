...
codegen/arrai/tests/%.nodiff: $(ARRAI_OUT)/% $(TIDY)
	# Ignore dbendpoints, since it's not generated by Sysl transforms.
	# TODO: Remove this check altogether once Sysl transforms are replaced.
	([ dbendpoints == $* ] || diff -rwuBq $(TEST_OUT_DIR)/$* $</ | awk 'BEGIN { err = 0 } END { exit err } /^Files .* differ$$/ { print; err = 1 }') && touch $@
