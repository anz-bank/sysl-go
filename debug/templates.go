...
func writeTrace(w http.ResponseWriter, traceId string, m Metadata) error {
	...
	var err error
	for _, e := range es {
		currentSvg, err = updateSvg(currentSvg, e, be)
		if err != nil {
			logrus.Error(err)
		}
	}
...
func updateSvg(svg string, e Entry, be Entry) (string, error) {
	currentSvg := svg
	var textsArg string
	var err error
	...
	if e.ServiceName == be.ServiceName {
		...
		currentSvg, err = UpdateSvg(currentSvg, textsArg, reqColor(e.Response.StatusCode))
		if err != nil {
			logrus.WithError(err).Error("upstream inbound svg update failed")
		}

		textsArg = fmt.Sprintf(`{'<-- %s %s %s'}`, e.ServiceName, e.Request.Method, e.Request.Route)
		currentSvg, err = UpdateSvg(currentSvg, textsArg, resColor(e.Response.StatusCode))
		if err != nil {
